...
class EventsCalendar : ViewPager, MonthView.Callback {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        try {
            ...
            mCurrentItem?.mPreviousArrow?.setOnClickListener {
                previousPage()
            }
            mCurrentItem?.mNextArrow?.setOnClickListener {
                nextPage()
            }
    ...
    fun goToToday(day: Calendar, minDate: Calendar, smoothScroll: Boolean = true) {
        mSelectedMonthPosition = EventsCalendarUtil.getMonthPositionForDay(day, minDate)
        currentItem = mSelectedMonthPosition
            ...
            SINGLE_SELECTION -> {
                ...
                if (isPagingEnabled) {
                    ...
                    if (EventsCalendarUtil.currentMode == EventsCalendarUtil.MONTH_MODE) {
                        position =
                            EventsCalendarUtil.getMonthPositionForDay(selectedDate, mMinMonth)
                        setCurrentItem(position, false)
                        // post {
                        EventsCalendarUtil.monthPos = currentItem
                        EventsCalendarUtil.selectedDate = selectedDate
                        mCalendarMonthsAdapter?.getItem(currentItem)
                        ...
                        setCurrentItem(position, false)
                        post {
                            mCalendarWeekPagerAdapter.getItem(currentItem)
                                ?.setSelectedDate(selectedDate!!)
                            doFocus = true
                        }
                    }
